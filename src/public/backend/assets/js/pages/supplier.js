$(document).ready(function () {
  filtering();

  // Form Validations
  $("#moduleModalForm").submit(function (e) {
    e.preventDefault();
  }).validate({
    errorElement: "i",
    errorClass: "msg msg-error",
    rules: {
      name: {
        required: true,
        rangelength: [3, 100],
      },
      phone: {
        required: true,
        rangelength: [10,10],
      },
      company_name: {
        required: true,
        rangelength: [3, 100],
      },
      address: {
        required: true,
        rangelength: [3, 100],
      },
      company_phone: {
        required: true,
        rangelength: [10,10],
      },
      company_email: {
        required: true,
        email: true,
        rangelength: [3, 100],
      },
      status: {
        required: true,
      },
    },
    messages: {
      name: {
        required: "Please enter name",
        rangelength: "Name should be 3 to 100 characters.",
      },
      phone: {
        required: "Please enter phone",
        rangelength: "phone should be 10 characters.",
      },
      company_name: {
        required: "Please enter company_name",
        rangelength: "company_name should be 3 to 100 characters.",
      },
      address: {
        required: "Please enter address",
        rangelength: "address should be 3 to 100 characters.",
      },
      company_phone: {
        required: "Please enter company_phone",
        rangelength: "company_phone should be 10  characters.",
      },
      company_email: {
        required: "Please enter company_email",
        rangelength: "company_email should be 3 to 100 characters.",
      },
      status: {
        required: "Please select status.",
      },
    },
    submitHandler: function (form) {
      let data = new FormData(form);
      ajaxJqueryValidateSubmit(form, data);
      return false;
    },
  });

  $("#openModalButton").click(function () {
    $("i.msg-error").hide();
    $("#moduleModalForm")[0].reset();

    $("#moduleModalForm").attr(
      "action",
      `${SITE_URL}/admin/${MODULE_CONTROLLER}`
    );
    $("#formMethod").val("POST");

    $("#formHead").text("Add Supplier");
    $("#moduleFormModal").modal("show");
  });

  // Show Edit modal
  $(document).on("click", ".edit-button", function () {
    let id = $(this).data("id");
    $.get(`${SITE_URL}/admin/${MODULE_CONTROLLER}/${id}/edit`, function (res) {
      if (res.code == 1) {
        $("i.msg-error").hide();
        $("#moduleModalForm")[0].reset();

        $("#moduleModalForm").attr(
          "action",
          `${SITE_URL}/admin/${MODULE_CONTROLLER}/${id}`
        );
        $("#formMethod").val("PUT");

        $("#name").val(res.data.name);
        $("#phone").val(res.data.phone);
        $("#company_name").val(res.data.company_name);
        $("#address").val(res.data.address);
        $("#company_phone").val(res.data.company_phone);
        $("#company_email").val(res.data.company_email);
        $("#status").val(res.data.status);

        $("#id").val(res.data.id);
        $("#formHead").text("Edit Supplier");

        $("#moduleFormModal").modal("show");
      }
    });
  });
});

// Populating Modules Table via jquery datatable using ajax
function filtering() {
  dataTable = $("#moduleTable").DataTable({
    oLanguage: {
      sEmptyTable: "No Brands Available",
    },
    columnDefs: [
      {
        orderable: false,
        targets: [],
      },
    ],
    searching: false,
    processing: true,
    serverSide: true,
    ajax: {
      url: `${SITE_URL}/admin/${MODULE_CONTROLLER}/fetch`,
      dataSrc: "data",
      data: function (d) {
        return $.extend({}, d, {
          search_keyword: $("#searchKeyword").val().toLowerCase(),
        });
      },
    },
    drawCallback: function () {
      // feather.replace();
    },
    lengthMenu: [10, 20, 30],
    columns: [
      {
        data: function (data, type, row, meta) {
          return meta.row + meta.settings._iDisplayStart + 1;
        },
        title: "Sl",
      },
      {
        data: function (data) {
          return `
          <a href="javascript:void(0)"  class="btn btn-warning btn-sm edit-button" data-id="${data.id}">Edit</a>
          <a href="javascript:void(0)"  class="btn btn-danger btn-sm delete-button" data-id="${data.id}">Delete</a>
          `;
        },
        title: "Action",
      },
      {
        data: "name",
        title: "Name",
      },
      {
        data: "phone",
        title: "phone",
      },
      {
        data: "company_name",
        title: "company_name",
      },
      {
        data: "address",
        title: "address",
      },
      {
        data: "company_phone",
        title: "company_phone",
      },
      {
        data: "company_email",
        title: "company_email",
      },
      {
        data: "status_formatted",
        title: "Status",
      },
    ],
  });
}
