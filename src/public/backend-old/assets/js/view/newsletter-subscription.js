$(document).ready(function () {
    ...
    $("#openModalButton").click(function () {
        ...
        $("#formMethod").val("POST");
        $("#formHead").text("Add Newsletter Subscription");
...
const filtering = () => {
    dataTable = $("#moduleTable").DataTable({
        oLanguage: {
            sEmptyTable: "No Subscriptions Available",
        },
        dom: "Bfrtip",
        buttons: [
            "pageLength",
        ...
        columns: [
            ...
            },
            {
                data: function (data) {
                    ...
                    return `<div style="cursor:pointer" class="status-change" data-name="${data.email}" data-id="${data.id}" data-status="${s}" ><span class="badge ${c} radius:tiny size:tiny padding:tiny">${t}</span></div>`;
                },
                title: "Status",
            },
        ],
...
function showEditModal(id) {
    $.get(`${SITE_URL}admin/${MODULE_CONTROLLER}/${id}/edit`, function (res) {
        if (res.code == 1) {
            ...
            $("#formHead").text("Edit Newsletter Subscription");
            $("#id").val(res.data.id);
