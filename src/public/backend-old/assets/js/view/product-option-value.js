$(document).ready(function () {
    ...
    $("#modalForm").validate({
        ...
        messages: {
            ...
            value: {
                ...
                minlength: "Value must be minimum 3 characters long",
                maxlength: "Value must be maximum 200 characters long",
                remote: "Value already used",
            },
            sequence: {
    ...
    $("#openModalButton").click(function () {
        ...
        $("#formHead").text("Add Product Option Value");
        openRightModal();
    });
});
...
const filtering = () => {
    dataTable = $("#moduleTable").DataTable({
        oLanguage: {
            sEmptyTable: "No values Available",
        },
        dom: "Bfrtip",
        buttons: [
            "pageLength",
        ...
        columns: [
            ...
            },
            {
                data: function (data) {
                    ...
                    return `<div style="cursor:pointer" class="status-change" data-name="${data.value}" data-id="${data.id}" data-status="${s}" ><span class="badge ${c} radius:tiny size:tiny padding:tiny">${t}</span></div>`;
                },
                title: "Status",
            },
        ],
...
function showEditModal(id) {
    ...
    $.get(`${SITE_URL}admin/${MODULE_CONTROLLER}/${id}/edit`, function (res) {
        if (res.code == 1) {
            ...
            $("#id").val(res.data.id);
            $("#formHead").text("Edit Product Option Value");
